import { For, mergeProps, onCleanup, onMount, Show, } from "solid-js";
...
const DragDropDebugger = () => {
    ...
    return (<Portal mount={document.body}>
      <For each={Object.values(state.droppables)}>
        {(droppable) => droppable ? (<Highlighter id={droppable.id} layout={droppable.layout} active={droppable.id === state.active.droppableId}/>) : null}
      </For>
      <For each={Object.values(state.draggables)}>
        {(draggable) => draggable ? (<Highlighter id={draggable.id} layout={draggable.layout} active={draggable.id === state.active.draggableId} color="blue" style={{
      ...
      <Show when={state.active.overlay} keyed>
        {(overlay) => (<Highlighter id={overlay.id} layout={overlay.layout} active={true} color="orange" style={{
